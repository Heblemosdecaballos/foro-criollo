        ...
        {media?.length ? media.map((m: any) => {
          ...
          return (
            <div key={m.id} className="border rounded-lg overflow-hidden">
              ...
              {m.media_type === "video" && (!isYouTube || !ytId) && <video src={url} controls className="w-full h-auto" />}
            </div>
          );
        ...
        <ul className="space-y-3">
          {comments?.length ? comments.map((c: any) => (
            <li key={c.id} className="border rounded p-3">
              <div className="text-xs muted-date">{new Date(c.created_at).toLocaleString()}</div>
              <div>{c.content}</div>
            </li>
