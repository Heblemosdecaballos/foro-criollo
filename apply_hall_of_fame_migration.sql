-- Apply Hall of Fame Migration
-- This script adds the Hall of Fame tables to the existing database
...
DROP POLICY IF EXISTS "horse_gallery_delete_admin" ON public.horse_gallery;
...
DROP POLICY IF EXISTS "gallery_comments_read_all" ON public.gallery_comments;
CREATE POLICY "gallery_comments_read_all" ON public.gallery_comments FOR SELECT USING (true);

DROP POLICY IF EXISTS "gallery_comments_insert_auth" ON public.gallery_comments;
...
DROP POLICY IF EXISTS "gallery_likes_read_all" ON public.gallery_likes;
...
DROP POLICY IF EXISTS "comment_likes_insert_auth" ON public.comment_likes;
...
-- Crear bucket de storage para medios si no existe
INSERT INTO storage.buckets (id, name, public) VALUES ('media', 'media', true) ON CONFLICT DO NOTHING;

-- Política de storage para permitir subida de archivos a usuarios autenticados
...
CREATE POLICY "Allow admins to delete media" ON storage.objects FOR DELETE USING (bucket_id = 'media' AND is_admin(auth.uid()));
